...
namespace ARMarker
{
    ...
    public class LayerResizeXYHandler : BaseWorkLayerHandler, IPointerDownHandler, IDragHandler
    {
        ...
        private void Select(Vector2 position)
        {
            base.Select();
            ...
            for (int i = 0; i < 4; i++)
            {
                if (Vector2.Distance(position, Camera.main.WorldToScreenPoint(corners[i])) < cornerDetectionRadius * 100) // Adjust sensitivity
                {
        ...
        {
            Debug.LogWarning("OnPointerDown eventData pos " + eventData.position);
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if ((selectedCorner != -1) && (selectedCorner != 3))
            {
